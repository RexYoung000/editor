      ...
      server.middlewares.use(async (req, res, next) => {
        ...
        if (pathname === '/api/library/search') {
          try {
            ...
            const quickTagId = urlObj.searchParams.get('quickTag') as LibraryQuickTagId | null;
            const quickTag = quickTagId
              ? LIBRARY_QUICK_TAGS.find((tag) => tag.id === quickTagId)
              : null;
            if (quickTagId && !quickTag) {
              return jsonError(res, 400, '不支持的快捷检索标签');
            }
            ...
            return jsonOk(res, {
              ...
              quickTags: LIBRARY_QUICK_TAGS.map((tag) => ({
                id: tag.id,
                label: tag.label,
                group: tag.group,
                primary: tag.primary,
