...
项目交接或新负责人接手时，先阅读 [项目交接总览](documentation/handoff.md)，再按其中的实时状态清单核对 Git、GitHub、部署和外部依赖。交接文档只记录当前核对时点，不能替代远端查询。
...
- 拖拽、吸附、动画、空间布局、连续数值、复杂状态切换或其他仅靠文字和静态截图难以确认手感的需求，在存在实质交互不确定性时，优先制作最小可操作的对话内交互 Demo。
- Demo 只验证交互规则、信息层级和视觉反馈，不发展为项目外的第二套正式实现，也不作为产品源码或正式交付物提交。
- 流程为：需求对齐 → 最小交互 Demo → Rex 即时体验 → 决策写回对应设计文档 → 项目实装 → 真实环境验收。
- 小型修复、规则明确的界面调整或无需体验判断的任务不强制制作 Demo。
- Demo 不能替代自动测试、浏览器真实页面或 Electron 真实环境；Laya 画布、性能、本地文件、远程模式、预览导出和最终集成结果仍按 [测试与验收](docs/testing.md) 完成对应验证。
