...
[Issue #137](https://github.com/RexYoung000/editor/issues/137) 统一 `KlInputImage` 与 `FractionInput` 的输入文字表现。两类输入框都在自身属性中选择黄色、蓝色或绿色字体图主题，新建组件默认蓝色；主题只影响输入框文字，不跟随已绑定键盘的皮肤，也不反向修改键盘。三种主题复用键盘文字已有的字体、颜色和描边定义，并通过同一套字形图集生成规则输出。绑定自定义答案键盘时只从答案取得所需字符，仍按输入框自己的主题和字号生成字体图。

“调整字号与间距”弹窗使用输入框真实普通态底图和 `sizeGrid=10,10,10,10` 九宫格显示当前框体，不用普通 CSS 边框近似。普通输入框直接编辑测试文本；分数输入框使用可视化的数字、分数结构和混排编辑。测试文本始终受当前输入位数和可输入字符约束，修改规则后立即同步裁切，保证左侧展示的是运行时允许出现的内容。右侧可同步调整输入位数或最大字符数、可输入字符、主字号与字符间距；主字号范围为 `12–120px`，字符间距范围为 `-20–60px`，两者步进均为 `1px`，数值框和滑杆双向同步。字符间距为负时收紧、为正时加宽，并统一作用于普通输入框连续字符、分数输入框外层字符、分子分母，以及字符与分数结构之间的间隔，不再为分数内部建立第二套间距。数值框允许清空后重新输入，只在形成有效范围内数值时实时更新，失焦时再归一化。

分数输入框额外提供独立“分数字号比例”，默认 `64%`，范围 `40%–100%`、步进 `1%`。该比例只控制分子、分母相对主字号的大小，并同步调整分数线宽度、上下间距和分数结构占位；分数外的普通数字继续使用主字号。分数结构按当前分子、分母的实际位数动态占位，不固定预留 4 位宽度；分子、分母不重复绘制完整输入框底图，只在当前编辑区域显示轻量选中边框，避免内层框体越出外层输入框。数字与数学符号使用适配窄字符的字形单元宽度，连续数字和分数之间保留可辨识但不过大的间隔。框体后续等比缩放时，主字号和分数字号仍共同同比变化，不把比例转换成第二套绝对像素值。

应用弹窗时一次保存主题、字号、字符间距、输入规则和测试样例，取消时不改变组件。字号和字符间距都以保存时框体为比例基准；之后在画布等比缩放框体时，普通字符、运算符、字符间距和分数结构按相同比例缩放，弹窗显示的实际值随之更新。测试样例仅用于编辑器内再次打开弹窗时还原预览，不写入 `fontClipValue`、正确答案、课件画布、预览或正式导出。

没有输入框主题配置的历史元素继续使用其已有 `fontClipSkin`、`sheet` 和显示比例，不因新建组件的蓝色默认值被静默迁移。只有显式拥有新主题配置的输入框才进入动态字体图生成链路。
...
forge 的普通图片组件支持单图镜像操作。单独选中图片后，右侧属性面板“变换”区域提供“左右镜像”和“上下镜像”两个执行按钮；每次点击切换对应方向，同一方向执行两次恢复原方向，并各自产生一条撤销记录。镜像状态保存在 `Element.props.mirrorX` / `mirrorY`，不修改原始素材；多选、混合选择和锁定图片不执行镜像。

镜像以图片自身方向为参照。编辑画布、预览和导出统一把镜像状态转换为 Laya `scaleX` / `scaleY`，并按图片当前尺寸、锚点和旋转角度补偿运行坐标，使可见外框、元素位置、旋转角度、选区和缩放控制点保持不变。`Image` 与当前工具栏创建的 `NewImage` 共用这套普通图片规则，其他以图片作为皮肤的业务组件不自动获得镜像能力。
...
[Issue #74](https://github.com/RexYoung000/editor/issues/74) 已提供“数字与小数点”和“分数输入”两种预设。[Issue #91](https://github.com/RexYoung000/editor/issues/91) 在同一预设体系中新增六种组合：

- 小数与分数组合：0–9、小数点、结构化分数、删除。
- 数学表达式：0–9、小数点、结构化分数、`+`、`−`、`×`、`÷`、`=`、`(`、`)`、删除。
- 数字百分比：0–9、`%`、删除。
- 数字百分比与小数点：0–9、`%`、小数点、删除。
- 数字百分比与运算符：0–9、`%`、`+`、`−`、`×`、`÷`、`=`、`(`、`)`、删除。
- 数字百分比、运算符与小数点：上述百分比、运算符与括号，并增加小数点。

现有两种和新增六种数学键盘均支持黄色、蓝色、绿色三种整套主题，默认黄色；旧课件没有主题字段时仍按黄色显示。属性面板复用自定义答案键盘的三色色块、选中态和实时更新方式，不能增加第二套主题选择交互。选择器缩略图统一展示默认黄色主题；四种百分比预设紧跟“数字与小数点”，两种新增分数预设紧跟“分数输入”，“自定义答案”保持在最后。

无运算符键盘使用 3 列，普通键帽固定为 `84×88`：12 键排 4 行；13 键排 5 行，删除键横跨末行 3 列。带运算符键盘使用 5 列 4 行；不足 20 键时删除键横跨剩余列。底板随行列扩展，并统一使用 `53,52,57,52` 完整九宫格，键帽尺寸、视觉范围和点击热区都不得超出底板有效内容区。

三色主题共用同一布局和字符映射。底板、普通/按下键帽、宽键和箭头复用已注册的同体系资源；数字、百分号、运算符、小数点和分数图标按主题使用独立字形或图标层。删除键由可拉伸键帽底图与独立居中的主题删除图标组成，图标保持原始比例，不随跨列键帽拉伸。
...
Issue #91 只增加键盘预设和主题，不改变现有键盘选择、自动绑定、页面内复用和 `camp` 分配逻辑。输入长度、字符占位、删除、小数点和结构化分数编辑继续由对应输入框的现有规则负责；百分号作为普通可见字符输出，不自动换算，也不新增表达式求值或数学等价判断。

Issue #157 将四种百分比键盘的 `%` 改为黄、蓝、绿三套正式图片。每套主题的普通态与按下态复用同一张图片，按下反馈继续由键帽状态与 2px 下移表达；按键输出、答案数据和历史课件结构不变。新建和历史百分比键盘在新版编辑器中统一使用新资源，已经发布的旧课件包只有重新导出后才会变化。绿色百分号的主体与描边必须跟同主题数字、运算符保持一致，不得使用验收中发现的黄色主体原图。

#### 自定义答案键盘

[Issue #112](https://github.com/RexYoung000/editor/issues/112) 在现有键盘选择器中增加“自定义答案键盘”，用于学生不能自由打字、只能从老师配置的文本答案中点选的填空场景。

[Issue #147](https://github.com/RexYoung000/editor/issues/147) 取消首版 9 项答案上限，把数量是否适合题目和画布的决定交给老师。

- 每个键盘实例独立保存至少 2 个答案，不设置答案数量上限，也不通过新增代码预设或逐题切图承载题目内容。老师根据题目需要和画布中的实际键盘尺寸自行控制数量；保存、重开、预览和正式导出不得截断答案。
...
- 布局由系统生成，老师不拖动单键、不调整单键尺寸：
  - 每个答案键帽根据自身字符数计算宽度，普通态和按下态皮肤通过九宫格横向拉伸；
  - 答案键帽使用统一字号，长答案通过增加键帽宽度容纳，不能为了塞入固定底板而缩小字号；
  - 答案键按配置顺序优先向前排满，清空键作为最后一个按键共同参与自动分行；清空键允许单独位于末行，分行确定后从自身左边线横向拉伸至底板右侧内容边线，单独成行时铺满整行，内部清空图标保持居中且不随皮肤拉伸；
  - 系统在键盘可用宽度内依次优先减少总行数、缩窄底板、排满靠前的答案行，再根据各行实际占宽选择更均衡的保序组合，不要求等宽、等列或固定每行数量；
  - 各行使用相同左边线；底板宽度取最宽一行加左右内边距，不能逐行居中。
- 自动分行必须使用可随答案数量稳定增长的计算方式，不能枚举所有分行组合；10、20、50 项仅作为回归样例，不构成新的产品上限。
- 键盘沿用数学键盘的键帽尺寸体系。底板宽度和高度根据最终最宽行与实际行数动态计算，并使用完整九宫格同时横向、纵向伸缩；顶部与底部按皮肤有效内容区分别保留安全边距，末行键帽不得侵入底框高光和阴影。
- 答案较多时键盘可以按真实布局超出固定课件画布；编辑器不分页、不滚动、不缩小键帽，也不设置二次上限，老师通过画布实况决定是否减少答案。
...
- 编辑器右侧答案配置中，每个答案输入框的可见区域必须与真实输入热区完全重合；序号、输入框及两者之间的空隙共同作为该答案的输入焦点区，以兼容系统大光标的箭头尖端与可见图形不重合。上移、下移、删除按钮使用独立布局区域，不能覆盖输入焦点区。鼠标进入输入框任意位置都应显示文本光标，点击答案项可直接切换焦点。
- 编辑器画布、预览和正式导出共用同一套文字图片生成规则：按“主题 + 答案文本 + 当前键帽宽度”把键帽文字渲染为透明 PNG，并复用缓存结果；文字图片只随键帽位置移动，不能随九宫格皮肤一起横向变形。
...
本能力不包含分页或滚动键盘、自由行列布局、单键尺寸编辑、连续词语组句，以及用户自定义字体或混搭主题资源。
...
- 工具栏名称为“文本”。点击后在当前可见画布中央创建组件并保持普通选中态，不自动进入输入；新组件的 `props.text` 写入真实默认内容“**双击编辑文本**”，默认文字颜色为白色以适配编辑器标准黑色画布，并正常进入历史、缩略图、预览和导出。点击画布、属性面板、工具栏或其他组件不会删除新建文本。
...
- 编辑态使用原生插入光标、选区、中文输入法和浏览器撤销；编辑区域与鼠标明确使用 I-beam，插入光标使用独立高对比强调色。双击默认内容“**双击编辑文本**”进入编辑时全选全部文字，直接输入即可替换；双击其他已有文本仍按点击位置放置光标。`Cmd/Ctrl+Enter`、`Esc` 或点击编辑区域外退出，画布、属性面板、工具栏等区域均适用；清空后退出仍保留空文本组件。编辑态拖动用于选择文字，不能移动组件；选中态拖动文本框内部可移动组件。
- 一次进入到退出只产生一条画布历史。普通文本和编辑中的输入底板保持透明，不增加会发布的默认底板。普通选中态继续使用蓝白辅助框；编辑态改用稳定屏幕粗细的青白双层描边、外圈强调和深色投影，在深浅背景和不同缩放下都能与选中态区分。固定宽高发生溢出时使用橙色外描边提示。
- 支持整段字体、字号、颜色、粗体、斜体、左中右对齐和行距；不支持局部混排、列表、链接、下划线和完整富文本。
- 预设字体使用单层列表，“思源黑体 Regular”是新建文本和无效历史字体 ID 的统一默认值；旧方正字体 ID 在课件打开时自动迁移。字体注册、资源与兼容规则见 [字体库与历史兼容](font-library.md)。
...
编辑器在“常用组件 → Spine 动画”中先打开预设选择器，保留“空白 Spine”并增加“手指点击”和“手指移动”两个独立预设。单击卡片只形成选中态，点击“添加组件”后才创建；取消不得新增元素或写入资源。空白项继续使用 `400 × 400` 和本地/资源库导入流程；两个手指预设复用同一套内置 Spine 3.7.89 资源，均按约 `225 × 428`、位置 `100,100` 创建。“手指点击”只使用 `game_an1`，“手指移动”只使用 `game_an2`，两者分别添加为独立 Spine 元素，不能通过动画下拉框互相替换。

两个手指预设卡片分别显示对应动画的编辑器专用静态缩略图。编辑画布和课件运行时都不自动播放；右侧属性的播放按钮对所有 Spine 元素执行一次试听，结束后回到第一帧并暂停，再次点击从头重播。课件运行时需要播放时继续使用现有“播放动画”事件动作显式配置，不为预设自动增加触发关系。
