...
“调整字体大小”弹窗使用输入框真实普通态底图和 `sizeGrid=10,10,10,10` 九宫格显示当前框体，不用普通 CSS 边框近似。普通输入框直接编辑测试文本；分数输入框使用可视化的数字、分数结构和混排编辑。测试文本始终受当前输入位数和可输入字符约束，修改规则后立即同步裁切，保证左侧展示的是运行时允许出现的内容。右侧可同步调整输入位数或最大字符数、可输入字符与主字号，主字号数值框和滑杆双向同步，范围为 `12–120px`、步进为 `1px`，同时显示相对基准字号的百分比；数值框允许清空后重新输入，只在形成有效范围内数值时实时更新，失焦时再归一化。
...
应用弹窗时一次保存主题、字号、输入规则和测试样例，取消时不改变组件。字号以保存时框体为比例基准；之后在画布等比缩放框体时，普通字符、运算符和分数结构按相同比例缩放，弹窗显示的实际字号随之更新。测试样例仅用于编辑器内再次打开弹窗时还原预览，不写入 `fontClipValue`、正确答案、课件画布、预览或正式导出。
