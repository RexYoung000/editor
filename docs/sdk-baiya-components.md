...
- 布局由系统生成，老师不拖动单键、不调整单键尺寸：
  ...
  - 4 个答案使用两列两行；
  - 5～9 个答案使用三列网格；
  - 同一个键盘内的所有答案键帽按本题最长答案计算统一宽度，普通态和按下态皮肤通过九宫格横向拉伸；
  - 答案键帽使用统一字号，长答案通过增加键帽宽度容纳，不能为了塞入固定底板而缩小字号；
  - 答案按等宽阵列排列，每行整体居中；底板根据完整阵列宽度通过九宫格横向拉伸，不能反向压缩键帽或破坏行列对齐；
  - 清空键固定占最后一行。
- 键盘沿用数学键盘的键帽尺寸体系，并以 `330×420` 作为底板最小尺寸；答案与清空键作为一个整体在底板内垂直居中。
...
- 编辑器画布、预览和正式导出共用同一套文字图片生成规则：按“主题 + 答案文本 + 本题统一键帽宽度”把键帽文字渲染为透明 PNG，并复用缓存结果；文字图片只随键帽位置移动，不能随九宫格皮肤一起横向变形。
