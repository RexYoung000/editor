...
- 布局由系统生成，老师不拖动单键、不调整单键尺寸：
  ...
  - 4 个答案使用两列两行；
  - 5～9 个答案使用三列网格；
  - 每个答案键帽按自身文字长度计算目标宽度，允许同一行宽窄混排；普通态和按下态皮肤通过九宫格横向拉伸；
  - 每行根据实际键帽宽度整体居中，目标总宽超出底板可用区域时按该行比例收敛，不能溢出或遮挡相邻键帽；
  - 清空键固定占最后一行。
...
- 编辑器画布、预览和正式导出共用同一套文字图片生成规则：按“主题 + 答案文本 + 最终键帽宽度”把键帽文字渲染为透明 PNG，并复用缓存结果；文字图片只随键帽位置移动，不能随九宫格皮肤一起横向变形。
