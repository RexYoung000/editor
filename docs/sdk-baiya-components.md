...
2026-07-21 已确认新预设复用旧课件键盘资源：键盘外框、普通/按下键帽、数字和小数点字形、删除图标、分数按钮、分数线均作为内置运行资源注册。分数输入框外框继续复用 `klInput` 普通、聚焦、错误三态皮肤；禁用态使用普通态降低透明度并去饱和，同时关闭输入响应，不增加独立切图。分子/分母焦点高亮由运行时状态动态绘制。
