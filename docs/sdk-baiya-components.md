...
Issue #157 将四种百分比键盘的 `%` 改为黄、蓝、绿三套正式图片。每套主题的普通态与按下态复用同一张图片，按下反馈继续由键帽状态与 2px 下移表达；按键输出、答案数据和历史课件结构不变。新建和历史百分比键盘在新版编辑器中统一使用新资源，已经发布的旧课件包只有重新导出后才会变化。
...
编辑器在“常用组件 → Spine 动画”中先打开预设选择器，保留“空白 Spine”并增加“手指点击”。单击卡片只形成选中态，点击“添加组件”后才创建；取消不得新增元素或写入资源。空白项继续使用 `400 × 400` 和本地/资源库导入流程；手指预设使用内置 Spine 3.7.89 资源，按约 `225 × 428`、位置 `100,100` 创建，默认选中 `game_an1`，同时保留 `game_an2`。

预设卡片只显示编辑器专用静态缩略图。编辑画布和课件运行时都不因选中动画而自动播放；右侧属性的播放按钮对所有 Spine 元素执行一次试听，结束后回到第一帧并暂停，再次点击从头重播。课件运行时需要播放时继续使用现有“播放动画”事件动作显式配置，不为预设自动增加触发关系。
