...
- 标尺（水平 + 垂直，百分比定位）
- Ctrl+S 保存
- 元素列表支持修饰键多选；画布统一命中、框选、整体移动、缩放旋转与拖动复制已完成前两批重构

后续实施与验收以 [画布与图层基础交互整体优化设计](./roadmap/canvas-layer-interaction-design.md) 为准，不再把存在数据分支等同于真实用户能力完成。

2026-07-15 第一批基础交互已完成：统一 Pointer Events 命中与拖动链路、首次手势直接拖动、多选整体移动、父子选择规范化、旋转感知命中、默认相交框选和无变化不记历史已通过自动检查与 Electron 真实画布验收。运行容器无论是否已有子元素，都以低对比度常驻线框表达边界；内部点击穿透到普通组件，没有组件时作为空白画布框选。

2026-07-15 第二批多选变换与拖动复制已完成：单选与多选统一变换框、八手柄自由/等比缩放、围绕选择中心整体旋转、角度吸附、跨旋转父容器变换及拖动复制的取消和单条历史链路已通过 39 项单元测试。Electron 真实画布已验证统一多选框、自由缩放、整体旋转和旋转后单选框方向；组合修饰键拖动由自动测试覆盖。智能吸附、参考线和完整图层面板仍待后续批次。
