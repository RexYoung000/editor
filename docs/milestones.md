...
2026-07-16 第四批距离提示与辅助设置已按交互预验收完成实现、待 Electron 视觉验收：画布顶部集中提供网格显示、网格吸附、智能吸附、参考线和距离提示五项全局偏好与恢复默认；普通拖动只测量明确同行或同列的最近元素，三元素等距时切换为两段相等距离与辅助吸附，斜对角不显示十字测距；`Alt/Option` 悬停可在不移动时查看距离。完整 69 项单元测试、Lint 与生产构建通过。
