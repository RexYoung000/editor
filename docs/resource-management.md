...
Agent 只能搜索现有 Forge 资源库，或导入老师在当前对话中明确提供的文件路径。MCP 不提供任意本机目录浏览接口；导入时继续复用课件本地复制、内容去重和资源路径规则，课程数据不得保存来源绝对路径。

老师提供 PDF 且资源库未命中时，Agent 可以在自身环境中裁取必要题图，再作为明确授权图片导入。题干应保持为可编辑文本，不能用整页 PDF 截图替代可交互题目。完整流程见 [Agent 作业草稿制作与 Forge MCP](./ai-authoring.md)。
