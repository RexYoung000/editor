...
> 本文档只覆盖旧版点击命中的局部修正，其中“多选拖动当前已支持”“框选相交无需修改”等判断已经被桌面实测和代码审计推翻。后续选择、拖动、框选、父子规范化、旋转命中和图层顺序统一以 [`docs/roadmap/canvas-layer-interaction-design.md`](../../roadmap/canvas-layer-interaction-design.md) 为准，本文不再作为实施依据。
