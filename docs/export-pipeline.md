...
`bakeCourseAssets` 在同一份课程副本中统一烘焙 `KlInputImage`、`FractionInput` 和自定义答案键盘的文字资源。显式配置了新输入框文字主题的组件按“字符表 + 黄色/蓝色/绿色主题 + 当前比例字号”生成横向 FontClip PNG，并把运行时需要的 `fontClipSkin`、`sheet`、单元宽高与缩放参数写入副本。分数输入框还把独立分数字号比例转换为分子分母缩放、分数线宽度和纵向结构比例，默认 `64%`；普通输入框和分数输入框必须调用同一套样式与字形生成器，自定义答案键盘只提供答案字符集合，不能覆盖输入框自己的主题。
