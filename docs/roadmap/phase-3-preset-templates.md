...
现有单题讲解、练习题、二级弹窗、图文混排、拖拽和选择题只作为候选方向，不等同于已经确认的首批清单。完成真实课件样本和老师使用频率分析后，由产品负责人确认最终 5 个结构，再按一个模板一个 Issue 创建实施任务。每个模板 Issue 必须独立说明适用场景、默认结构、可编辑范围、资源来源和真实预览验收方式。
